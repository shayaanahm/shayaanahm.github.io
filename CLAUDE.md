...
- **`world-scrub.js`** is the `#world` section: a scroll-scrubbed **pre-rendered camera flight** (scroll sets `video.currentTime`) through four clay-diorama islands — automation workshop → martial-arts ring → CV book tower → contact lighthouse. Built with the **scroll-world skill** pipeline: stills + clips generated with **Higgsfield** (`seedance_2_0_mini`, 720p — the Starter-plan model; `seedance_2_0` needs Pro), encoded per the skill (desktop `-g 8` crf20 / mobile `-m.mp4` 720p `-g 4` crf23), assets in `world/`. The engine is the skill's reference scrub engine adapted to run as an **embedded section**: a sticky 100vh `.sw-view` + tall `.sw-track` instead of fixed full-page chrome, and all scroll math offset by the container's document top. Config (copy, clips, connectors, pacing) is the inline `mountScrollWorld(...)` call at the end of index.html — copy is verbatim from the old world, don't invent claims.
  - **Seam rule (critical if regenerating clips):** every connector's `--start-image`/`--end-image` must be the neighbouring dive clips' ACTUAL extracted frames, never the stills — otherwise the seam pops. One model for the whole chain. Prompts/frames preserved procedure: see the scroll-world skill.
  - The `world/*.webp` stills are posters, the lazy-load fallback, and the whole experience under `prefers-reduced-motion` (clips are never fetched there — stills cross-dissolve).
  - Regenerating costs Higgsfield credits (~7/image, ~20/video on Starter; 2 concurrent jobs max). The rendered clips themselves cost nothing to serve.
