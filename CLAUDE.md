...
## The scroll engine — `scroll.js`, `home.css`, `world.js`

`index.html` loads `style.css` → `home.css`, then `scroll.js` → `script.js` → `world.js`. **Load order matters**: `home.css` overrides `style.css`, and `script.js` still owns reveals.
...
- **`world.js` + the `.room` sections** are `#world`: four full-viewport "process rooms" (Discovery lounge → Design studio → Build lab → Handover study; the last introduces the owner, CTAs → `#about` / contact). Each room's clip **PLAYS at natural speed when the room enters the viewport** (blob-loaded, plays once per visit, holds the last frame, pauses off-screen); CSS scroll-snap clicks room-to-room. **Do NOT reintroduce scroll-scrubbing** (`video.currentTime` driven by scroll): it was built twice and rejected — stepping through AI-generated frames exposes the models' morphing artifacts ("looks AI-generated") and stutters on modest hardware regardless of encoding. Natural playback hides both.
  - Clips are **cinematic photoreal** (the clay-diorama look was rejected as childish), generated with **Higgsfield** as one continuous forward take: each leg's `--start-image` = the previous leg's ACTUAL extracted last frame. `seedance_2_0_mini` for leg 1; `kling3_0` for legs 2-4 (Seedance's NSFW filter false-positives on interiors; rejected jobs aren't billed; Starter plan = 2 concurrent jobs, `seedance_2_0` is Pro-gated).
  - Encode: native 720p/24, **no motion interpolation** (interpolated in-betweens shimmer = "AI look"), `-g 2`, crf 20 desktop / 960px crf 23 `-m.mp4` mobile. `world/*.webp` posters are the legs' first frames — the pre-play state and the whole experience under `prefers-reduced-motion` (video never loads there).
  - **A CSS-layer trap lives in this page:** style.css's universal `* { padding: 0 }` reset is unlayered, and unlayered author styles beat ALL `@layer` rules — any component that injects layered CSS will silently lose every padding/margin. Inject unlayered.
